...
export function ConfirmDeleteModal() {
    const { modalIsOpen, setModalIsOpen, onDeleteComment, commentToDelete } = useContext(SectionContext);

    if (!modalIsOpen) {
        return null;
    }

    const onClickOnOverlayOrCancelButton = () => {
        setModalIsOpen!(false);
    };
    const onDialogClick = (event: React.MouseEvent<HTMLDivElement>) => {
